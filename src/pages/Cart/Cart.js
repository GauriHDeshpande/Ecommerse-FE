...
const Cart = () => {
  ...
  const init = async () => {
    ...
    setProductDetails(data.productsSelected);
  }
  ...
  const removeProductFromCart = async (productId) => {
    ...
    // update cart by removing the item from cart
                    ...
                    productDetails.length > 0 ? productDetails.map(product => (
                      ...
                      <div className="order-details-product" key={product.id}>
                        <div className="order-details-product-img">
                          <img src="https://img.favpng.com/8/17/0/product-design-clip-art-logo-food-png-favpng-TsCQEsJH2LUYN3d5Q6RzrTsqL.jpg" alt="Product" />
                        </div>
                        ...
                        <div className="order-details-product-actions">
                          <div className="order-details-product-remove btn btn-info" onClick={() => removeProductFromCart(product.id)}>
                            Remove
                          </div>
                        </div>
                      </div>
                ...
                </div>
                {
                  productDetails.length > 0 && (
                    ...
                    <div className="price-details">
                      ...
                      <Link to="/" className="continue-shopping-btn btn btn-info">Continue Shopping</Link>
                      <Link to="/checkout" className="checkout-btn btn btn-success">Checkout</Link>
                    </div>
                  )
                }
              </div>
